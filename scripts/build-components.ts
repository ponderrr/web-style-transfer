...
program
  ...
  .option('-o, --output <path>', 'Output directory for component library', './components')
  ...
  .action(async (specPath: string, options: any) => {
    try {
      console.log(chalk.blue(`🏗️  Building component library from specification: ${specPath}`));
      ...
      // Load specification
      const spec: UnifiedSpec = JSON.parse(await fs.readFile(specPath, 'utf-8'));
      ...
      switch (options.framework) {
        ...
        case 'svelte':
          ...
          break;
        default:
          console.error(chalk.red(`❌ Unsupported framework: ${options.framework}`));
          process.exit(1);
      }
      ...
      console.log(chalk.gray(`📝 TypeScript: ${options.typescript ? 'Yes' : 'No'}`));
    } catch (error) {
...
async function generateReactComponents(spec: UnifiedSpec, options: any): Promise<void> {
  const outputDir = options.output;
  const useTypeScript = options.typescript;

  for (const [componentName, componentSpec] of Object.entries(spec.generation.components)) {
    const fileExtension = useTypeScript ? 'tsx' : 'jsx';
    const componentContent = generateReactComponent(componentName, componentSpec, useTypeScript);

    await fs.writeFile(path.join(outputDir, `${componentName}.${fileExtension}`), componentContent);

    console.log(chalk.gray(`   ✅ Generated ${componentName}.${fileExtension}`));
  }
}
...
function generateReactComponent(
  ...
  const propTypes = generatePropTypes(componentName, componentSpec, useTypeScript);
...
function generatePropTypes(
  ...
  const propDefinitions = Object.entries(requiredProps)
    .map(([key, value]: [string, any]) => `  ${key}: ${getTypeScriptType(value)};`)
    .join('\n');
...
function generateComponentLogic(componentName: string, componentSpec: any): string {
...
async function generateVueComponents(_spec: UnifiedSpec, _options: any): Promise<void> {
  // Placeholder for Vue component generation
  console.log(chalk.yellow('   ⚠️  Vue component generation not yet implemented'));
}

async function generateSvelteComponents(_spec: UnifiedSpec, _options: any): Promise<void> {
  // Placeholder for Svelte component generation
  console.log(chalk.yellow('   ⚠️  Svelte component generation not yet implemented'));
}

async function generateIndexFile(spec: UnifiedSpec, options: any): Promise<void> {
  const outputDir = options.output;
  const useTypeScript = options.typescript;
...
`;
  ...
  await fs.writeFile(path.join(outputDir, `index.${fileExtension}`), indexContent);
}
