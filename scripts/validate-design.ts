...
program
  ...
  .description(
    "Validate design consistency and quality against S-tier standards"
  )
  .version("1.0.0")
  ...
  .action(async (specPath: string, options: any) => {
    try {
      ...
      if (!specData.tokens || !specData.patterns) {
        console.error(
          chalk.red("❌ Invalid spec format: missing tokens or patterns")
        );
        process.exit(1);
      }
      ...
      console.log(
        chalk.gray(`Overall Score: ${qualityScore.overall.toFixed(2)}/1.0`)
      );
      ...
      if (qualityScore.overall < threshold) {
        console.log("");
        console.log(
          chalk.red(`❌ Quality score below threshold (${threshold})`)
        );
